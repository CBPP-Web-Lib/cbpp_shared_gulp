...
openssl req -newkey rsa:2048 -nodes -keyout vm-localhost.key -subj "/C=US/O=CBPP/CN=vm-localhost" -out vm-localhost.csr
openssl x509 -req -extfile <(printf "subjectAltName=DNS:vm-localhost") -days 9999 -in vm-localhost.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out vm-localhost.crt